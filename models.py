...
class FCNet(nn.Module):
    def __init__(self, dims, c_dim, units, activation, norm):
        ...
        for i in range(len(all_units)-2):
            self.net['LinearBlock{}'.format(i+1)] = LinearBlock(all_units[i], all_units[i+1], activation, norm, bias=True)
        self.net['fc'] = nn.Linear(all_units[-2], all_units[-1], bias=True)
        
        self.names = list(self.net.keys())
...
class ConvNet(nn.Module):
    def __init__(self, dims, c_dim, units, activation, norm):
        ...
        for i in range(len(units)):
            ...
            self.net['ConvBlock{}'.format(i+1)] = ConvBlock(c_in, c_out, k, s, p, activation, norm, bias=True)
            h, w = int((h - k + 2*p) / s) + 1, int((w - k + 2*p) / s) + 1
        self.net['fc'] = nn.Linear(c_out * h * w, c_dim, bias=True)
        
        self.names = list(self.net.keys())
    
    def forward(self, x):
        for i in range(len(self.names)-1):
            x = self.net[self.names[i]](x)
        
        # flatten for CONV -> FC layer
        x = x.view(x.shape[0], -1)
        x = self.net[self.names[-1]](x)
        return x
